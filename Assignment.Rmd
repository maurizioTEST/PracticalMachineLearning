...
```
So we reduced to the variable that are present both in the training set and in the test set!
...
selected      
```
Now we check how the model fit the training and test set
```{r}
...
Let us evaluate now the model using the selected variables.
```{r}
set.seed(23)
...
```
achieving an accuracy 0f 0.9939.
